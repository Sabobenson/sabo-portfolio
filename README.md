# Sabo Benson
Telecommunication Engineering Graduate  

## About Me
I am a telecommunication engineering focused on network design, routing systems, wireless communication and infrastructure solutions.

My goal is to build reliable network and connectivity solutions for businesses and institutions.

---

## Skills

- Network Design
- IP Addressing & Subnetting
- Routing and Switching
- Wireless Networking
- Network Simulation

---

## Tools

- Cisco Packet Tracer
- MikroTik RouterOS
- Wireshark
- GNS3

---

## Projects

### 1. Basic Routing Between Two Networks
Simulation showing communication between two LAN networks using a router.

Status: In Progress

---

### 2. Small Office Network Design
Design of office network including DHCP, switching and internet sharing.

Status: Planned

---

### 3. Wireless Network Deployment
Design of WiFi infrastructure for small businesses.

Status: Planned

---

## Contact

Email: sabobenson20@gmail.com  
Location: Tanzania
